window.onload = function () {
    ...
    document.querySelector("#submitForm").addEventListener('click', function () {
        ...
        if (document.querySelectorAll("form input").forEach((item) => { item.value != "" || item.value != " " })) {
            document.querySelectorAll(".set-tag-content").forEach((item, index) => {
                if (countTag == 3) {
    ...
    // 移除
    document.querySelector("#removeTag").addEventListener("click", () => {

        // 元素列表
        var checkedElement = [];
        document.querySelectorAll("[name=tag-select-box]").forEach((item) => {
            item.checked ? checkedElement.push(item) : "";
        });

        // 移除选定的元素
        function removeSelectElement(checkedElement) {
            checkedElement.map((item) => {
    ...
    // 全选
    window.flag = false;
    document.querySelector("#select-all-tag").addEventListener("click", () => {
    ...
    axios({
        ...
        for (var a = 0; a < response.length; a++) {
            template = template + `<div class="article-group">
                ...
                <div class="article-item">${response[a].articleTitle}</div>
            </div>`;
        }
    ...
    document.querySelector(".select-all-article").addEventListener("click", () => {
    ...
    document.querySelector(".delete-article").addEventListener("click", () => {
        var checked = [];
        document.querySelectorAll(".article-group input[type=checkbox]").forEach((item) => { item.checked ? checked.push(item) : "" });
        // 全选状态
